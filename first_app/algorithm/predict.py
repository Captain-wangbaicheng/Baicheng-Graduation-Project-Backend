...
# 将系统用户上传的数据作为'user_test'


def predict_function(train1, test):
    train = train1
    train_labels = train['emd_lable2']
    train.drop('emd_lable2', axis=1, inplace=True)

    result_data = pd.DataFrame()
    result_data['pass_id'] = test['pass_id']
    result_data['emd_lable'] = test['emd_lable2']
    ...
    result_data['predict_next'] = test_pred[:, 1]  # 数据类型：'numpy.ndarray'
    result_data['pax_fcny_dense'] = test['pax_fcny_dense']  # 机票费
    result_data['pax_tax_dense'] = test['pax_tax_dense']  # 机票税费
    # 最后一次乘机至今时长
    result_data['diff_recent_flt_day_dense'] = test['diff_recent_flt_day_dense']
    result_data['tkt_3y_amt_dense'] = test['tkt_3y_amt_dense']  # 最近3年机票总消费
    # 最近3年总里程
    result_data['dist_all_cnt_y3_dense'] = test['dist_all_cnt_y3_dense']
    # 最近3年机票金额-国际
    result_data['tkt_i_amt_y3_dense'] = test['tkt_i_amt_y3_dense']
    # 最近3年机票总金额
    result_data['tkt_all_amt_y3_dense'] = test['tkt_all_amt_y3_dense']
    # 最近2年付费选座次数
    result_data['select_seat_cnt_y2_dense'] = test['select_seat_cnt_y2_dense']
    # 最近3年常飞城市的平均旋回半径
    result_data['avg_pref_city_radius_y3_dense'] = test['avg_pref_city_radius_y3_dense']
    # 最近2年每次旅行平均里程
...
# print(predict_function(user_train, user_test))
